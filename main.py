...
td_mults = [(1/golden**0.15)**i for i in range(16)]
print(td_mults)
...
insts = [Instrument(str(i), 24 + 2*i , 60 + 2*i, i+1, cols[inds[i]], td_mults[::-1][i], 'Trumpet') for i in range(12)]
...
piece = Piece(dur_tot, chord, insts, nos, rsw, rhythm_nCVI_max, td_max, td_oct, \
        max_vel, rr_max, rdm, rsm, rtd_min, rr_min)
